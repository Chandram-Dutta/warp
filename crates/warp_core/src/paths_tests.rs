...
fn local_only_uses_distinct_config_and_application_namespaces() {
    ...
    let dirs = project_dirs_for_app_id(AppId::new("dev", "warp", "WarpLocalOnly"), None).unwrap();
    let legacy = project_dirs_for_app_id(AppId::new("dev", "warp", "WarpOss"), None).unwrap();
    assert_ne!(dirs.config_local_dir(), legacy.config_local_dir());
