...
#[test]
fn gui_bundle_uses_resolved_state_and_ignores_cli_logs() {
    let tmp = tempfile::tempdir().unwrap();
    let state = log_state(tmp.path(), LogFrontend::Gui, "warp_preview.log");
    write_bytes(
        &state.log_directory.join("warp_preview.log"),
        b"active gui session",
    );
    write_bytes(
        &state.log_directory.join("warp_preview.log.old.0"),
        b"previous gui session",
    );
    write_bytes(
        &state.log_directory.join("warp_preview.log.in_session.0"),
        b"mid-session gui chunk",
    );
...
#[test]
fn crash_recovery_paths_use_channel_name_in_gui_directory() {
    ...
    assert_eq!(
        crash_recovery_process_log_file_path(&gui_dir, "warp_dev.log"),
        gui_dir.join("warp_dev.log.recovery")
    );
}
