...
use super::{
    ...
    VerticalTabsSummaryData, branch_label_display, coalesce_summary_branch_entries,
    ...
    push_normalized_unique_summary_text, search_fragments_contain_query,
    ...
    terminal_pull_request_badge_label, terminal_search_text_fragments, uses_outer_group_container,
...
fn summary_pane_kind_icons_pick_two_oldest_distinct_pane_kinds() {
    assert_eq!(
        select_summary_pane_kind_icons([
            ...
            (EntityId::from_usize(20), SummaryPaneKind::Settings),
            (
        ...
        Some(SummaryPaneKindIcons::Pair {
            ...
            secondary: SummaryPaneKind::Settings,
        })
    );
}
...
fn summary_pane_kind_icons_recompute_when_oldest_kind_is_removed() {
    assert_eq!(
        ...
        Some(SummaryPaneKindIcons::Pair {
            primary: SummaryPaneKind::Settings,
...
#[test]
fn detail_sidecar_supports_terminal_and_warp_drive_object_panes() {
...
fn terminal_primary_line_uses_last_completed_command_when_shell_title_matches_working_directory() {
    let line =
        terminal_primary_line_data("~/warp", "~/warp", Some("cargo nextest run".to_string()));
...
fn terminal_primary_line_falls_back_to_new_session() {
    let line = terminal_primary_line_data("~/warp", "~/warp", None);
...
fn terminal_search_fragments_include_rendered_terminal_badges() {
    ...
    assert!(search_fragments_contain_query(&fragments, "terminal"));
...
#[test]
fn primary_labels_normalize_and_deduplicate_without_reordering() {
    let mut values = Vec::new();
    let mut seen = std::collections::HashMap::new();
    push_normalized_unique_summary_text(&mut values, &mut seen, "zsh");
    push_normalized_unique_summary_text(&mut values, &mut seen, "  cargo   test  ");
    push_normalized_unique_summary_text(&mut values, &mut seen, "\t ");
    push_normalized_unique_summary_text(&mut values, &mut seen, "cargo test");

    assert_eq!(values, vec!["zsh", "cargo test"]);
}
...
fn summary_search_fragments_include_hidden_overflow_values() {
    ...
    assert!(search_fragments_contain_query(&fragments, "zsh"));
