...
#[test]
fn local_input_visibility_follows_command_lifecycle() {
...
#[test]
fn local_input_visibility_during_shell_reinitialization() {
    let mut model = TerminalModel::mock(None, None);
    model.block_list_mut().reinit_shell();
    model.simulate_long_running_block("bash --norc", "");
...
#[test]
fn local_input_visibility_excludes_alternate_screen_even_during_bootstrap() {
    let mut model = TerminalModel::mock(None, None);
    model.enter_alt_screen(true);
    assert!(!model.is_input_box_visible());

    model.exit_alt_screen(true);
    assert!(model.is_input_box_visible());
...
#[test]
fn local_input_visibility_excludes_exited_shell() {
    let mut model = TerminalModel::mock(None, None);
...
#[test]
fn local_terminal_rejects_dcs_hook_with_unregistered_session_id() {
    let mut terminal = TerminalModel::mock(None, None);
    terminal.start_command_execution();
    terminal.command_finished(CommandFinishedValue {
        completion_metadata: CompletionMetadata {
