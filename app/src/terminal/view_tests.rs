...
#[test]
fn ctrl_c_from_shared_viewer_forwards_unchanged() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        let terminal = add_window_with_terminal(&mut app, None);
        let pty_writes: Rc<RefCell<Vec<Vec<u8>>>> = Rc::new(RefCell::new(Vec::new()));
...
#[test]
fn ctrl_c_from_shared_viewer_rejected_by_agent_in_control() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        let terminal = add_window_with_terminal(&mut app, None);
        let pty_writes: Rc<RefCell<Vec<Vec<u8>>>> = Rc::new(RefCell::new(Vec::new()));
...
#[test]
fn conversation_metadata_cannot_replace_local_terminal_pane_title() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        ...
        terminal.update(&mut app, |view, ctx| {
            view.terminal_title = "/local/project".to_owned();
...
fn cloud_mode_setup_v2_suppresses_sharer_input_updates_while_followup_setup_commands_run() {
    App::test((), |mut app| async move {
        ...
        terminal.update(&mut app, |view, ctx| {
            ...
            {
                let model = view.model.lock();
                assert!(model.is_input_box_visible());
            }
...
#[test]
fn ctrl_c_interrupts_local_command_through_action_and_input_event() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        let terminal = add_window_with_terminal(&mut app, None);
        ...
        terminal.update(&mut app, |view, ctx| {
            view.handle_input_event(&InputEvent::CtrlC, ctx);
        });
        assert_eq!(*pty_writes.borrow(), vec![vec![C0::ETX]]);
    });
}
...
#[test]
fn ctrl_c_at_idle_prompt_does_not_write_to_pty() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        let terminal = add_window_with_terminal(&mut app, None);
        let pty_writes = Rc::new(RefCell::new(Vec::new()));
...
fn paste_raw_image_clipboard_in_cli_agent_sends_correct_bytes() {
    fn run_for_agent(agent: CLIAgent) {
        App::test((), move |mut app| async move {
            ...
            terminal.update(&mut app, |view, ctx| {
                CLIAgentSessionsModel::handle(ctx).update(ctx, |sessions, ctx| {
                    sessions.set_session(view.view_id, CLIAgentSession { agent }, ctx);
                });
