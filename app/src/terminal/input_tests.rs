...
use crate::ai::blocklist::{BlocklistAIPermissions, ResponseStreamId};
...
use crate::settings::{
    AliasExpansionSettings, AppEditorSettings, InputBoxType, PrivacySettings, WarpPromptSeparator,
};
...
#[test]
fn test_command_history_session_order() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        ...
        let only_history_commands = history_commands
            .clone()
            .into_iter()
            .sorted_by(|a, b| a.cmp(b, Some(current_session_id)))
            .collect::<Vec<_>>();
...
fn ampersand_stays_shell_text() {
    App::test((), |mut app| async move {
        ...
        for (shell, command) in [
            (ShellFamily::Posix, "printf background &"),
            (ShellFamily::PowerShell, "& ./local-command.ps1"),
        ] {
            input.update(&mut app, |input, ctx| {
                input.clear_buffer_and_reset_undo_stack(ctx);
                input
                    .editor
                    .update(ctx, |editor, _| editor.set_shell_family(shell));
            });
            for character in command.chars() {
                input.update(&mut app, |input, ctx| {
                    input.user_insert(&character.to_string(), ctx);
                });
            }
            input.read(&app, |input, ctx| {
                assert_eq!(input.buffer_text(ctx), command);
                assert_eq!(
...
#[test]
fn slash_prefixed_text_remains_literal_shell_input() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        let terminal = add_window_with_bootstrapped_terminal(&mut app, None, None).await;
        let input = terminal.read(&app, |terminal, _| terminal.input().clone());
        ...
        input.update(&mut app, |input, ctx| {
            input.user_insert("plan src/main.rs", ctx);
        });
        input.read(&app, |input, ctx| {
            assert_eq!(input.buffer_text(ctx), "/plan src/main.rs");
            assert_eq!(
                input.ai_input_model().as_ref(ctx).input_type(),
                InputType::Shell
            );
...
#[test]
fn test_shell_prompt_layout_ignores_legacy_agent_input_style() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        let terminal = add_window_with_bootstrapped_terminal(&mut app, None, None).await;
        ...
        for (honor_ps1, same_line, expected) in [
            (false, false, false),
            (false, true, true),
            (true, false, true),
        ] {
            SessionSettings::handle(&app).update(&mut app, |settings, ctx| {
                ...
                settings
                    .saved_prompt
                    .set_value(
                        PromptConfiguration::from_chips([], same_line, WarpPromptSeparator::None)
                            .into(),
                        ctx,
                    )
                    .unwrap();
            ...
            terminal.read(&app, |terminal, ctx| {
                let mut model = terminal.model.lock();
                model.set_honor_ps1(honor_ps1);
                assert_eq!(should_render_prompt_on_same_line(&model, ctx), expected);
            });
        }
    });
}
...
#[test]
fn test_at_sign_remains_literal_shell_input() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        ...
        input.update(&mut app, |input, ctx| {
            input.user_insert("printf '%s' @repo user@host @scope/pkg", ctx);
        });

        input.read(&app, |input, ctx| {
            assert_eq!(
                input.buffer_text(ctx),
                "printf '%s' @repo user@host @scope/pkg"
            );
            assert_eq!(
                input.ai_input_model().as_ref(ctx).input_type(),
                InputType::Shell
...
fn local_only_input_actions_preserve_next_command_but_reject_agent_entrypoints() {
    ...
    assert!(Event::OpenFileInEditor(PathBuf::from("notes.txt")).is_available_in_product());
...
#[test]
fn prompt_file_editor_request_preserves_directory_and_filename() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        let session_info = SessionInfo::new_for_test();
        let session_id = session_info.session_id;
        ...
        let input = terminal.read(&app, |terminal, _| terminal.input().clone());
        let requested_paths = Rc::new(RefCell::new(Vec::new()));
        let observed_paths = requested_paths.clone();
        app.update(|ctx| {
            ctx.subscribe_to_view(&input, move |_, event: &Event, _| {
                if let Event::OpenFileInEditor(path) = event {
                    observed_paths.borrow_mut().push(path.clone());
                }
            });
        });
        input.update(&mut app, |input, ctx| {
            input.set_active_block_metadata(
                BlockMetadata::new(Some(session_id), Some("/tmp/project with spaces".into())),
                true,
                ctx,
            );
            input.handle_prompt_event(
                &PromptDisplayEvent::OpenTextFileInEditor("notes; literal.txt".into()),
                ctx,
            );
        });
        assert_eq!(
            requested_paths.borrow().as_slice(),
            [PathBuf::from("/tmp/project with spaces/notes; literal.txt")]
        );
    });
}
...
#[test]
fn cli_agent_session_events_preserve_local_shell_input() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        let terminal = add_window_with_bootstrapped_terminal(&mut app, None, None).await;
        let input = terminal.read(&app, |terminal, _| terminal.input().clone());
        input.update(&mut app, |input, ctx| {
            input.replace_buffer_content("printf '%s' '@literal'", ctx);
        });
        ...
        input.read(&app, |input, ctx| {
            assert_eq!(input.buffer_text(ctx), "printf '%s' '@literal'");
