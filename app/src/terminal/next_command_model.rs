//! Local Next Command suggestion model.
...
use warp_local_ai::{
    DirectNextCommandProvider, LocalAIError, NextCommandProvider, ProviderConnection,
};
#[cfg(feature = "local_fs")]
...
use crate::terminal::next_command::{CommandContext, HistoryContext, get_recent_commands};
...
async fn generate_local_next_command_suggestion(
    ...
    let connection = connection.ok_or(LocalAIError::MissingModel)?;
    let suggestion = provider
        .suggest(connection, context.provider_context(prefix))
        .await?;
    Ok(suggestion.display_only_command().to_owned())
}
...
impl NextCommandSuggestionState {
    ...
    pub fn command_suggestion(&self) -> Option<&str> {
        match &self {
            ...
            NextCommandSuggestionState::Ready { suggestion, .. } => {
...
pub struct NextCommandModel {
    ...
    cached_zerostate_next_command_context: Option<CommandContext>,
...
impl NextCommandModel {
    ...
    pub fn cycle_next_command_suggestion(&mut self) {
    ...
    fn get_next_command_context(
        ...
        let recent_commands = get_recent_commands(terminal_model.clone(), 5);
        #[cfg(feature = "local_fs")]
                ...
                async move {
                    ...
                    if !next_command_context.history_contexts.is_empty() {
                        ...
                        for (most_likely_next_command, count) in &most_likely_next_commands {
                            ...
                            {
                                return (
                                    Ok(most_likely_next_command.to_owned()),
                                    prefix,
                    ...
                    for reverse_chronological_command in reverse_chronological_potential_autosuggestions.unwrap_or_default() {
                        if is_command_valid(&reverse_chronological_command.command, completion_context.as_ref(), session_env_vars.as_ref()).await {
                            return (
                                Ok(reverse_chronological_command.command),
                                Some(prefix),
                                next_command_context,
                            );
                        }
    ...
    fn on_next_command_suggestion_result(
        &mut self,
        result: (Result<String, LocalAIError>, Option<String>, CommandContext),
        ctx: &mut ModelContext<Self>,
    ) {
        self.next_command_abort_handle = None;
        let (result, prefix, next_command_context) = result;
        if prefix.is_none() {
        ...
        match result {
            Ok(suggestion) => {
                ...
                self.next_command_state = NextCommandSuggestionState::Ready { suggestion };
