...
///
/// Lists persisted workspace directories that are not yet present in the user's
/// `directory_tab_colors` with a non-`Suppressed` color, and exposes a pinned `+ Add directory…`
/// footer that falls back to the native folder picker.
///
/// Emits:
...
pub(super) struct DirectoryColorAddPicker {
    ...
    /// Used to short-circuit `refresh_items` when nothing relevant has changed, so repeated workspace
    /// events don't pay for per-path `exists()` + `canonicalize()` on every fire.
...
impl DirectoryColorAddPicker {
    pub(super) fn new(ctx: &mut ViewContext<Self>) -> Self {
        ctx.subscribe_to_model(&PersistedWorkspace::handle(ctx), |me, _, _, ctx| {
