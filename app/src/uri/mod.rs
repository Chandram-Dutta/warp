...
/// * Text files open in the configured external editor.
...
fn open_file(window_id: Option<WindowId>, path: PathBuf, ctx: &mut AppContext) {
    ...
    if action == OpenFileAction::Editor {
        #[cfg(feature = "local_fs")]
        {
            ...
            let target = resolve_file_target(&path, editor_settings);
            ...
            {
                workspace.update(ctx, |workspace, ctx| {
                    workspace.open_file_with_target(path, target, None, ctx);
                });
            }
        }
    } else {
