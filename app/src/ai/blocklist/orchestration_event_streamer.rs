...
impl OrchestrationEventStreamer {
    ...
    /// parent-self delivery (`ParentSelf`), then advances the cursor.
    ///
    ...
    /// forwarded to `handle_event_batch` so the parent's
    ...
    fn drain_family_events(
        ...
        self.persist_event_cursor(cursor_conversation_id, max_seq, ctx);
...
/// pill bar.
///
/// Working states collapse to `InProgress`, terminals map one-for-one, and the
